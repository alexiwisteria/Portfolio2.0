            ...
            children: (
                <div className="flex flex-wrap gap-2">
                    {["Java", "OOP", "Agile Methodologies"].map((skill) => (
                        <span key={skill} className="bg-gray-300 px-3 py-1 text-xxs xs:text-xs sm:text-sm rounded-lg">
                            {skill}
                        </span>
                    ))}
                </div>
...
const EXTRACURRICULARS: InfoItem[] = [
    {
        ...
        date: "Jan 2025",
        children: <p className="text-xxs xs:text-xs sm:text-sm">Collaborated on software projects, organized hackathons, and mentored peers.</p>,
    },
    {
        icon: ComputerDesktopIcon,
        title: "Studio E - Ensign College AI Autograding Project - Scrum Master",
        date: "Jan 2025",
        children: <p className="text-xxs xs:text-xs sm:text-sm">Led an AI-driven grading project, optimizing development speed by 40%.</p>,
    },
];
...
const EXPERIENCE: InfoItem[] = [
    {
        ...
        children: <p className="text-xxs xs:text-xs sm:text-sm">Improved ticket resolution efficiency and streamlined technical issue management.</p>,
    },
    {
        icon: UserGroupIcon,
        title: "Team Lead - Research and Business Development Center",
        date: "Apr 2020 - Jul 2020",
        children: <p className="text-xxs xs:text-xs sm:text-sm">Led a team in self-taught data analytics for actionable insights.</p>,
    },
];

export function InformationSection() {
    return (
        <section className="px-2 xs:px-4 sm:px-6 md:px-8 lg:px-10">
            <div className="container mx-auto max-w-6xl">
                {["Skills", "Education", "Extracurricular Activities", "Experience"].map((section, index) => (
                    <div key={index} id={section.toLowerCase().replace(/ /g, "-")} className="mb-6">
                        <h2 className="text-base xs:text-lg sm:text-xl md:text-2xl font-bold">{section}</h2>
